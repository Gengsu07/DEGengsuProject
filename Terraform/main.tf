...
resource "google_cloud_run_service" "mageai" {
  name     = var.app_name
  location = var.region
...
resource "google_cloud_run_service" "metabase" {
  name     = "${var.app_name}-metabase"
  location = var.region

  template {
    spec {
      containers {
        image = var.docker_image_metabase
        ports {
          container_port = 3000
        }
        resources {
...
resource "google_cloud_run_service_iam_member" "run_all_users" {
  service  = google_cloud_run_service.mageai.name
  location = google_cloud_run_service.mageai.location
  role     = "roles/run.invoker"
  member   = "allUsers"
}
resource "google_cloud_run_service_iam_member" "metabase_all_users" {
